        ...
        visas.map((visa: any) => (
          <Card key={visa.visa_name} className='overflow-hidden'>
            <Button
              variant='ghost'
              className={cn(
                'w-full flex justify-between items-center p-4 hover:bg-gray-50 hover:text-secondary',
                expandedVisa === visa.visa_name && 'bg-gray-50 text-secondary'
              )}
            ...
            {expandedVisa === visa.visa_name && <CardContent>{renderScoringCriteria(visa.criteras)}</CardContent>}
          </Card>
        ))
      ) : (
        <div>No visa criterias found</div>
      )}
    </div>
  );
}
